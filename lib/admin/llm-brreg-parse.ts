...
export type BrregTier2Output = {
  categories: CategoryAssignment[];
  rationale: string;
};

// Re-export the Tier 1 contract verbatim — same JSON shape as media.
// The orchestrator runs validatePhrases against `aktivitet` instead of
// `headline`; the validator itself is haystack-agnostic.
...
export function parseBrregTier2(content: string): BrregTier2Output | null {
...
// Brace-balanced scan that respects strings + escapes. Mirrors the
// helper in llm-media-parse (kept here too so this module is
