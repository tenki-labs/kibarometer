...
//
// The Tier 1 system prompt is hardcoded here in PR 2. PR 3 lands migration
// 0018 (llm_prompts table) and switches this orchestrator to read the prompt
// + few-shot examples from the DB so they become operator-editable.
//
...
// Norwegian + English. Verbatim-only extraction. Three few-shot examples
// (obvious-AI / not-AI / subtle-AI) inline; PR 3 will move these into
// llm_prompts.examples jsonb.
const TIER1_SYSTEM_PROMPT = `Du analyserer norske og engelske jobbannonser. Returner KUN gyldig JSON på formen:
...
Output: {"ai_relevant": true, "phrases": [{"text": "Data Scientist"}, {"text": "LLM"}]}`;
...
export type RunDiscoverResult = {
  ...
  reason?: "no_api_key" | "already_running";
  job_id?: string;
  metadata?: {
    processed: number;
    ai_relevant: number;
...
export async function runDiscover(args: {
  ...
  try {
    for (let idx = 0; idx < candidates.length; idx += 1) {
      ...
      try {
        const r = await processOne(sb, posting);
        processed += 1;
