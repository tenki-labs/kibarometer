...
// Module-level cache for the public token. NAV's /api/publicToken
// rotates "irregularly" (their docs); 30-min TTL is a safe assumption
// that still cuts the per-page token refetch down to ~2 calls/hour
// even during a 1000-page fast-forward batch. Resets to null on the
// first 401/403 from an actual feed request (handled by the caller —
// throwing an Error is enough; the next process restart re-fetches).
...
// Fetch one posting's detail. ACTIVE postings return an `ad_content` field
// with the full ad (description, jobtitle, workLocations, occupationCategories
// etc.). INACTIVE postings return only {uuid, status, sistEndret} — caller
// must check `status` before extracting detail fields.
