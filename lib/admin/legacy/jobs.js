...
const SWEEPABLE_JOB_NAMES = [
  ...
  // Archive enrichment (lib/admin/legacy/nav-archive.js). The weekly
  // indexer can legitimately run ~25 min, still under STALE_RUNNING_MS.
  "archive_index_nav",
  "archive_enrich_nav",
];
...
export async function reprocessNavPostings({ sb, trigger = "manual" }) {
  ...
  try {
    // Keyset pagination over the id PK (was deep OFFSET — see
    // keyset-paginate.js). nav_postings.id is a text PK (0007), unique +
    // NOT NULL, so the gt. seek never skips or duplicates a row.
    await keysetPaginate(
      sb,
      {
        path: `/nav_postings?select=id,title,description,is_ai,matched_keywords`,
        orderCol: "id",
        pageSize: PAGE,
      },
      async (rows) => {
        ...
        for (const r of rows) {
          ...
          const same =
            tags.is_ai === r.is_ai &&
            tags.matched_keywords.length ===
              (r.matched_keywords || []).length &&
  ...
  try {
    for (let idx = 0; idx < candidates.length; idx += 1) {
      ...
      try {
        ...
        // Provenance for the archive pipeline (0075): this text came from
        // NAV's feed API, not from a recovered capture.
        if (updates.description) updates.description_source = "nav_api";
