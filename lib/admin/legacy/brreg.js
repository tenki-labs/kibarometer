...
//
// `ingestMode` ('live' | 'backfill') gates Tier 1 eligibility downstream.
// fetchBrreg (daily cron) passes 'live'; bootstrapBrreg passes 'backfill'.
// Note: merge-duplicates means a row first stamped 'backfill' will flip to
// 'live' if a later daily-cron tick re-touches it — that's intended (the
// daily cron re-seeing a row means it's currently live in the source feed).
async function upsertCompaniesChunk(sb, chunk, ingestMode) {
...
export async function fetchBrreg({
  ...
  try {
    ...
    const [keywords, categoryRows, kommuneFylkeMap] = await Promise.all([
      loadActiveKeywords(sb, ["brreg", "any"]),
      loadCategoryRows(sb),
    ...
    const result = await fetchEnheterBatch({
      ...
      onPage: async (enheter, pageIdx, pageEnvelope) => {
        ...
        for (let i = 0; i < rows.length; i += UPSERT_BATCH) {
          ...
          upserted += await upsertCompaniesChunk(sb, chunk, "live");
        }
...
// ---- Backfill (bulk dump, PR 4 - renamed from bootstrap in PR 7) -------

// bootstrapBrreg({ sb, trigger, floorDate })
//   - floorDate (ISO YYYY-MM-DD or null): when set, only entities with
//     registreringsdatoEnhetsregisteret >= floorDate are persisted.
//     When null/undefined, the floor is read from app_settings
//     (default 2018-01-01 per migration 0047). Function name kept
//     so existing callers that still import it don't break.
//
...
// totalEnqueued }. floorDate is null when no filter is applied.
export async function bootstrapBrreg({
  ...
  try {
    // Resolve floor date. Explicit param wins; otherwise read from
    // app_settings (default 2018-01-01 per migration 0047). Null floor
    // means "no filter" — supported as an explicit override but the
    // settings row is not-null in production.
    let floor = floorDate;
    if (!floor) {
      ...
      floor = settings?.[0]?.brreg_bootstrap_floor_date || null;
    }
    ...
    await heartbeat(sb, job.id, {
      step: floor
        ? `downloading bulk dump (filter registrert_dato >= ${floor})`
        : `downloading bulk dump (full registry, no floor)`,
      pct: 1,
    });
    ...
    for await (const entity of parseJsonArrayObjects(gunzip)) {
      totalSeen++;
      // Floor filter: when null, accept everything. When set, require a
      // regDate field >= floor. Entities missing the field are dropped
      // either way — without a registration date we can't tell when the
      // entity was registered, so it doesn't fit the startup framing.
      const regDate = entity?.registreringsdatoEnhetsregisteret;
      if (!regDate) continue;
      if (floor && regDate < floor) continue;
      ...
      if (pendingChunk.length >= UPSERT_BATCH) {
        totalUpserted += await upsertCompaniesChunk(sb, pendingChunk, "backfill");
        pendingChunk = [];
        // Drain enqueue immediately AFTER the company upsert: every orgnr
        // in pendingEnqueue corresponds to a row that's now upserted, so
        // the brreg_url_queue.orgnr FK is satisfied. Doing this on the
        // heartbeat tick instead would race — heartbeats fire on time
        // (every 5s) while company flushes fire on batch size, so the
        // enqueue could refer to companies still buffered in pendingChunk.
      ...
      // Heartbeat-only tick (no DB writes here — those happen on batch flush).
...
// pruneExpiredBrregRoles({ sb }) — GDPR Art 5(1)(e) storage limitation.
// Deletes brreg_roles rows whose company has been slettet_dato older
// than app_settings.brreg_roles_retention_years (default 5 years).
// Aggregated founder-age statistics in brreg_snapshot_focus_daily survive
// the prune since they no longer reference identifiable individuals.
//
// Returns { expired_companies, roles_deleted }.
export async function pruneExpiredBrregRoles({ sb }) {
  const settings = await sb(
    `/app_settings?id=eq.1&select=brreg_roles_retention_years`,
    { service: true },
  );
  const retentionYears = settings?.[0]?.brreg_roles_retention_years ?? 5;

  // Compute cutoff: today - retentionYears
  const cutoff = new Date();
  cutoff.setUTCFullYear(cutoff.getUTCFullYear() - retentionYears);
  const cutoffIso = cutoff.toISOString().slice(0, 10);

  // Find expired company orgnrs.
  const expired = await sb(
    `/brreg_companies?slettet_dato=lt.${cutoffIso}&select=orgnr`,
    { service: true },
  );
  if (!expired.length) {
    return { expired_companies: 0, roles_deleted: 0 };
  }

  // Chunk DELETE by orgnr in.(...) to keep the URL bounded.
  const CHUNK = 500;
  let total = 0;
  for (let i = 0; i < expired.length; i += CHUNK) {
    const ids = expired.slice(i, i + CHUNK).map((e) => encodeURIComponent(e.orgnr));
    await sb(`/brreg_roles?orgnr=in.(${ids.join(",")})`, {
      service: true,
      method: "DELETE",
      prefer: "return=minimal",
    });
  ...
  return { expired_companies: expired.length, roles_deleted: total };
}
...
// jobs row for visibility on /admin/processes.
//
// Also runs the GDPR retention prune on personal data at the same
// nightly cadence — the snapshot refresh is the natural anchor since
// it's the only nightly brreg-pipeline tick. Best-effort: a prune
// failure doesn't fail the snapshot refresh.
//
// Returns { status, job_id, headline, prune }.
export async function refreshBrregSnapshots({ sb, trigger = "manual" }) {
  ...
  try {
    ...
    // GDPR retention prune (best-effort).
    let prune = { expired_companies: 0, roles_deleted: 0 };
    try {
      await heartbeat(sb, job.id, { step: "pruning expired personal data" });
      prune = await pruneExpiredBrregRoles({ sb });
    } catch (e) {
      console.error("pruneExpiredBrregRoles failed (non-fatal):", e.message);
    }
    ...
    await finishJob(sb, job.id, {
      status: "success",
      progress_pct: 100,
      metadata: { headline, prune },
    });
    return { status: "success", job_id: job.id, headline, prune };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await finishJob(sb, job.id, { status: "failed", error: msg.slice(0, 1000) });
    throw err;
