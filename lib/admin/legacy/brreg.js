...
//     When null/undefined, the full registry loads (no date filter).
//     The legacy app_settings.brreg_bootstrap_floor_date column was
//     deprecated in migration 0033 — readers see null and fall through
//     to the no-filter path. Function name kept so existing tests +
//     callers that still import it don't break.
//
...
export async function bootstrapBrreg({
  ...
  try {
    // Resolve floor date. Explicit param wins; otherwise check the legacy
    // app_settings column (deprecated by migration 0033, returns null after
    // that runs). Null floor = no filter, full registry loads.
    let floor = floorDate;
    if (!floor) {
