...
export async function runDiscover({
  ...
  try {
    for (const source of sources) {
      if (now() - startMs > WALL_TIME_MS) break;
      ...
      for (const item of items) {
        ...
        // ingest_mode is set on first-arrival only: media_url_queue has a
        // UNIQUE(url_hash) and we insert with ignore-duplicates, so a URL
        // already enqueued by media-backfill stays 'backfill' and won't
        // flip to 'live' even if RSS later re-discovers it.
        toEnqueue.push({
          source_id: source.id,
          url: canonical,
