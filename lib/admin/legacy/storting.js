...
import { sweepStaleRunningJobs } from "./jobs.js";
...
async function upsertSakerChunk(sb, chunk, ingestMode) {
  ...
  await sb(`/storting_saker?on_conflict=sak_id`, {
    ...
    // Upsert with merge-duplicates is idempotent — re-applying the same
    // batch yields the same row. Safe to retry through a 502 hiccup.
    retryTransient: true,
  });
  return body.length;
}
...
async function upsertVedtakChunk(sb, chunk, ingestMode) {
  ...
  await sb(`/storting_vedtak?on_conflict=vedtak_id`, {
    ...
    body,
    prefer: "return=minimal,resolution=merge-duplicates",
    // Idempotent upsert — safe to retry on transient 5xx, same reasoning
    // as the saker upsert above.
    retryTransient: true,
  });
  return body.length;
}
...
export async function backfillStorting({
  ...
  try {
    ...
    for (let i = 0; i < sessions.length; i++) {
      ...
      // Heartbeat lives inside the per-session try so a transient
      // PostgREST 502 on the bookkeeping PATCH lands in per_session[]
      // alongside any fetchStorting failure, rather than killing the
      // whole loop via the outer catch.
      try {
...
export async function retagStorting({ sb, trigger = "manual" }) {
  ...
  try {
    ...
    // Keyset pagination over the sak_id PK (was deep OFFSET — see
    // keyset-paginate.js). storting_saker.sak_id is a bigint PK (0064),
    // unique + NOT NULL, so the gt. seek never skips or duplicates a row.
    await keysetPaginate(
      sb,
      {
        path: `/storting_saker?select=sak_id,tittel,korttittel,emne_liste,raw_jsonb`,
        orderCol: "sak_id",
        pageSize: RETAG_PAGE_SIZE,
      },
      async (rows) => {
        ...
        const patch = rows
          .map((r) => {
            ...
            return {
              sak_id: r.sak_id,
              // tittel is NOT NULL with no default on storting_saker, so a
              // bare {sak_id, has_ai_*, ...} body trips the constraint during
              // PostgREST's INSERT … ON CONFLICT pre-check (NOT NULL is
              // validated before ON CONFLICT routes to DO UPDATE). Re-emit
              // the existing tittel so the always-conflicting INSERT row is
              // valid; the DO UPDATE then writes tittel = EXCLUDED.tittel as
              // a no-op against the same value.
              tittel: r.tittel,
    ...
    // Keyset pagination over the vedtak_id PK (bigint PK, 0064) — same
    // rationale as saker above.
    await keysetPaginate(
      sb,
      {
        path: `/storting_vedtak?select=vedtak_id,tekst,tittel,raw_jsonb`,
        orderCol: "vedtak_id",
        pageSize: RETAG_PAGE_SIZE,
      },
      async (rows) => {
        ...
        const pct =
          50 + Math.min(50, Math.round((vedtakProcessed / 10000) * 50));
        await heartbeat(sb, job.id, {
          pct,
