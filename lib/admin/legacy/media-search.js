...
//   }
//
// Queries are NOT in search_config — they come from the central keyword
// catalogue (`public.keywords` rows where domain in ('media','any') and
// status in ('canonical','trial')) and are passed in by media-backfill.
// This keeps every outlet on the same query set as /admin/keywords and
// avoids per-outlet drift.
//
...
export async function searchSourceUrls({
  ...
  if (!Array.isArray(queries) || queries.length === 0) {
    throw new Error("queries er tom (sjekk /admin/keywords for media-domene)");
  }
