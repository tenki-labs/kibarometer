...
//
// Date format: data.stortinget.no returns Microsoft JSON dates of the
// shape "/Date(1715000000000+0200)/" — NOT ISO 8601. Postgres rejects this
// verbatim ("invalid input syntax for type date"). parseStortingDate /
// parseStortingTimestamp below normalize both /Date(ms)/ and ISO strings
// so we tolerate either format if the API ever switches.
...
export function buildSakRow(sak, ctx) {
  ...
  const sist_oppdatert_dato = parseStortingDate(sak?.sist_oppdatert_dato);
...
export function buildVedtakRow(vedtak, ctx) {
  ...
  const dato_tid = parseStortingTimestamp(vedtak?.stortingsvedtak_dato_tid);
...
// Microsoft JSON date format: "/Date(ms)/" or "/Date(ms+offset)/" (also "-").
// Stortinget's eksport API emits these despite ?format=json — that took a
// failed backfill to discover. Returns a Date object, or null on no match.
...
// Normalize a Stortinget date field to YYYY-MM-DD. Accepts either the
// /Date(ms)/ format the live API emits, or an ISO 8601 string (what the
// docs page sample showed, and what the legacy test fixtures use).
// Returns null on anything we can't parse — caller stores null rather
// than failing the whole batch.
export function parseStortingDate(v) {
  if (!v) return null;
  const s = String(v);
  const d = parseMsDate(s);
  if (d) return d.toISOString().slice(0, 10);
  if (/^\d{4}-\d{2}-\d{2}/.test(s)) return s.slice(0, 10);
  return null;
}

// Same shape as parseStortingDate but returns a full ISO timestamp for the
// timestamptz columns (vedtak.dato_tid). On ISO input we pass through —
// Postgres parses the verbatim string itself.
export function parseStortingTimestamp(v) {
  if (!v) return null;
  const s = String(v);
  const d = parseMsDate(s);
