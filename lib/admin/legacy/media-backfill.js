...
//
// One tick = one wall-time-bounded run. Delegates URL discovery to the
// kiba-scraper sidecar (scrapegraph), enqueues new URLs into
...
// successful tick. Operators run the button repeatedly until the queue
// depth flattens.
//
...
export async function runMediaBackfill({
  ...
  sourceId,
  trigger = "manual",
  maxWallMs: _maxWallMs = DEFAULT_WALL_MS,
  now = () => Date.now(),
}) {
  ...
  const rows = await sb(
    `/media_sources?id=eq.${encodeURIComponent(sourceId)}` +
      `&select=id,name,domain,crawl_delay_ms,backfill_cursor`,
    { service: true },
  );
  ...
  try {
    ...
    const stats = { ...result.stats, adapter: "scrapegraph" };
