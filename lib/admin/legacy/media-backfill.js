...
import { canonicalizeUrl, urlHash } from "./media-processor.js";
...
export async function runMediaBackfill({
  ...
  try {
    ...
    let urls;
    let stats;
    if (source.backfill_method === "sitemap") {
      ...
      // New default. Delegates URL discovery to the kiba-scraper sidecar
      // which runs DuckDuckGo + Playwright + LLM-extracted URL parsing
      // on its own container. Same {urls, stats} shape so the rest of
      // this orchestrator is unchanged.
      const kwRows = await sb(
        "/keywords?status=in.(canonical,trial)&domain=in.(media,any)&select=term",
        { service: true },
      );
      const queries = (Array.isArray(kwRows) ? kwRows : [])
...
async function batchEnqueue(sb, sourceId, urls) {
  ...
  for (const u of urls) {
    ...
    seen.add(hash);
    rows.push({ source_id: sourceId, url: canonical, url_hash: hash });
  }
