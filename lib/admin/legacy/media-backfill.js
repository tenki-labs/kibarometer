...
export async function runMediaBackfill({
  ...
  try {
    ...
    if (source.backfill_method === "sitemap") {
      ...
      // Pull queries from the central keyword catalogue so every outlet
      // searches the same canonical+trial set. domain=in.(media,any)
      // mirrors lib/admin/legacy/media-processor.js's loadActiveKeywords.
