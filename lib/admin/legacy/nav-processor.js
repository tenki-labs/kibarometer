...
//
// Two callers:
//   1. processPayload({ sb, navRawRow, matchers, ingestMode }) — runs after
//      each fetch/backfill batch writes a nav_raw row. Upserts SUMMARY-tier
//      rows (title, employer, municipal, posted_at, status, source_url,
//      ingest_mode) and tags against title only. ingestMode is 'live' for
//      fetchNav and the daily backfillNav cron; 'backfill' only for the
//      historical fast-forward loop.
...
export function enrichFromDetail(detail) {
  // NAV wraps the ad payload under `ad_content` (verified live against
  // /api/v1/feedentry/{uuid}). An older revision of this code read
  // `detail.json`, which silently returned `{}` — every field came back
  // null, the upsert still wrote `detail_fetched_at`, and the snapshot
  // SQL (filtering `category is not null`) saw zero enriched rows.
  const j = detail?.ad_content || {};
...
//
// `ingestMode` ('live' | 'backfill') determines whether rows from this
// page are eligible for LLM Tier 1 (live only). Caller decides — fetchNav
// always passes 'live'; backfillNav passes 'backfill' only when
// trigger==='fast-forward'. Note: merge-duplicates means a row first
// stamped 'backfill' will flip to 'live' if a later daily-cron tick
// re-touches it — that's intended (the daily cron re-seeing a row means
// it's currently live in the source feed).
//
...
// since populated.
export async function processPayload({ sb, navRawRow, matchers, ingestMode }) {
...
export async function loadActiveKeywords(sb, domains = ["jobs", "any"]) {
  ...
  // domain filter scopes a tagger to its pillar's keywords + universal ones.
  // NAV defaults to (jobs, any); media-processor.js passes (media, any);
  // brreg.js / brreg-reprocess.js pass (brreg, any) so brreg-domain
  // keywords promoted via the candidate flow (0040_keyword_candidates_brreg)
  // are actually picked up by the BRREG matcher. Without an explicit
  // domains arg, any keyword tagged domain='brreg' or domain='media'
  // would be silently ignored by NAV.
  const csv = domains.join(",");
  return sb(
    `/keywords?status=in.(canonical,trial)&domain=in.(${csv})&select=term,language,category,match_type`,
    { service: true }
  );
}
