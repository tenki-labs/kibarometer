...
// then stale-attempt AI-flagged. One round-trip per pool keeps the
// PostgREST query simple.
async function selectCandidates(sb, k) {
  // Pool A: AI-flagged with no fetch state row at all.
  // Implemented as a left-anti-join via PostgREST's nested resource
  // pattern would be awkward; we fetch all fetch-state orgnrs and
  // filter client-side. The drain tick is small (K=20) so this is fine.
  const untouched = await sb(
    `/brreg_companies?is_ai_relevant=is.true` +
      `&select=orgnr` +
      `&order=registrert_dato.desc.nullslast` +
      `&limit=${Math.max(k * 4, 200)}`,
    { service: true },
  );
  if (!untouched.length) return [];

  const orgnrs = untouched.map((r) => r.orgnr);
  const inList = orgnrs.map(encodeURIComponent).join(",");
  const tried = await sb(
    `/brreg_financials_fetch_state?orgnr=in.(${inList})&select=orgnr,last_fetch_attempt_at`,
    { service: true },
  );
  ...
  for (const orgnr of orgnrs) {
    const ts = triedMap.get(orgnr);
    if (!ts) untouchedOrgnrs.push(orgnr);
    else if (new Date(ts) < cutoff) staleOrgnrs.push(orgnr);
  }
