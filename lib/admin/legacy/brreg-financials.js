...
export async function upsertFinancials(sb, rows) {
...
export async function upsertFetchState(sb, orgnr, status, error = null) {
...
export async function selectAllCandidates(sb) {
  ...
  return [...untouchedOrgnrs, ...staleOrgnrs];
}

export async function selectCandidates(sb, k) {
  const all = await selectAllCandidates(sb);
