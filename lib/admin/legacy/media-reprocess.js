...
export async function reprocessMediaArticles({ sb, trigger = "manual" }) {
  ...
  try {
    ...
    // keyset-paginate.js). media_articles.id is a uuid PK (0029), unique +
    // NOT NULL, so the gt. seek never skips or duplicates a row. The
    // deleted_at filter rides along in the base path.
